...
// CustomRoleBuilder builds a custom role.
...
// NewCustomRoleBuilder creates a new CustomRoleBuilder.
func NewCustomRoleBuilder() *CustomRoleBuilder {
...
// WithRole sets the name of a custom role.
func (b *CustomRoleBuilder) WithRole(role string) *CustomRoleBuilder {
	b.role = role
	return b
}

// WithDB sets the database of a custom role.
func (b *CustomRoleBuilder) WithDB(db string) *CustomRoleBuilder {
	b.db = db
	return b
}

// AddPrivilege adds a privilege to a custom role.
func (b *CustomRoleBuilder) AddPrivilege(
	db string,
	collection string,
	cluster bool,
	anyResource bool,
	actions []string) *CustomRoleBuilder {
	...
	b.privileges = append(b.privileges, Privilege{
		Resource: res,
		Actions:  actions,
	})
	return b
...
// AddRole adds an inherited role to a custom role.
func (b *CustomRoleBuilder) AddRole(role string, db string) *CustomRoleBuilder {
	b.roles = append(b.roles, Role{Role: role, Database: db})
	return b
}

// AddAuthenticationRestriction adds an authentication restriction to a custom role.
func (b *CustomRoleBuilder) AddAuthenticationRestriction(clientSource []string, serverAddress []string) *CustomRoleBuilder {
	restriction := AuthenticationRestriction{
		ClientSource:  clientSource,
		ServerAddress: serverAddress,
	}
	if b.authenticationRestrictions == nil {
		b.authenticationRestrictions = []AuthenticationRestriction{restriction}
	} else {
		b.authenticationRestrictions = append(b.authenticationRestrictions, restriction)
	}
	return b
}

// Build creates a new custom role based on the builder specification.
func (b *CustomRoleBuilder) Build() CustomRole {
	autoConfigRole := CustomRole{Role: b.role, DB: b.db, Privileges: b.privileges, Roles: b.roles}
	if b.authenticationRestrictions != nil {
		autoConfigRole.AuthenticationRestrictions = b.authenticationRestrictions
	}
	return autoConfigRole
}
