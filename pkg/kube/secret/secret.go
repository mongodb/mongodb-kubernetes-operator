...
type Deleter interface {
	DeleteSecret(ctx context.Context, objectKey client.ObjectKey) error
}
...
func CreateOrUpdateIfNeeded(ctx context.Context, getUpdateCreator GetUpdateCreator, secret corev1.Secret) error {
	...
	olsSecret, err := getUpdateCreator.GetSecret(ctx, types.NamespacedName{Name: secret.Name, Namespace: secret.Namespace})
	if err != nil {
