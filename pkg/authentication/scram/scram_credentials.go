...
func computeScram1AndScram256Credentials(username, password string) (*automationconfig.ScramCreds, *automationconfig.ScramCreds, error) {
	scram256Salt, err := generateSalt(sha256.New)
	if err != nil {
		return nil, nil, fmt.Errorf("error generating scramSha256 salt: %s", err)
	}

	scram1Salt, err := generateSalt(sha1.New)
	if err != nil {
		return nil, nil, fmt.Errorf("error generating scramSha1 salt: %s", err)
	}
