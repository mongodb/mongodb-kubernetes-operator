...
func TestDeadlockDetection(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-deadlocked.json"))
	assert.True(t, ready)
...
func TestNoDeadlock(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-no-deadlock.json"))
...
func TestNotReadyWaitingForRsReady(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-pending.json"))
	assert.False(t, ready)
	assert.NoError(t, err)
}
...
func TestNotReadyHealthFileHasNoPlans(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-no-plans.json"))
	assert.False(t, ready)
	assert.NoError(t, err)
}
...
func TestNotReadyHealthFileHasNoProcesses(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-no-processes.json"))
	assert.False(t, ready)
	assert.NoError(t, err)
}
...
func TestNotReadyMongodIsDown(t *testing.T) {
	...
	t.Run("Mongod is down for 90 seconds", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfigWithMongoUp("testdata/health-status-ok.json", time.Second*90))
		assert.False(t, ready)
		assert.NoError(t, err)
	})
	t.Run("Mongod is down for 1 hour", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfigWithMongoUp("testdata/health-status-ok.json", time.Hour*1))
		assert.False(t, ready)
		assert.NoError(t, err)
	})
	t.Run("Mongod is down for 2 days", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfigWithMongoUp("testdata/health-status-ok.json", time.Hour*48))
		assert.False(t, ready)
		assert.NoError(t, err)
	})
}
...
func TestReadyMongodIsUp(t *testing.T) {
	...
	t.Run("Mongod is down for 30 seconds", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfigWithMongoUp("testdata/health-status-ok.json", time.Second*30))
		assert.True(t, ready)
		assert.NoError(t, err)
	})
	t.Run("Mongod is down for 1 second", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfigWithMongoUp("testdata/health-status-ok.json", time.Second*1))
		assert.True(t, ready)
		assert.NoError(t, err)
	})
}
...
func TestReady(t *testing.T) {
	...
	ready, err := isPodReady(ctx, testConfig("testdata/health-status-ok.json"))
	assert.True(t, ready)
	assert.NoError(t, err)
}
...
func TestPodReadiness(t *testing.T) {
	...
	t.Run("Pod readiness is correctly checked when no ReplicationStatus is present on the file ", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfig("testdata/health-status-no-replication.json"))
		assert.True(t, ready)
		assert.NoError(t, err)
	})

	t.Run("MongoDB replication state is reported by agents", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfig("testdata/health-status-ok-no-replica-status.json"))
		assert.True(t, ready)
		assert.NoError(t, err)
	})

	t.Run("If replication state is not PRIMARY or SECONDARY, Pod is not ready", func(t *testing.T) {
		ready, err := isPodReady(ctx, testConfig("testdata/health-status-not-readable-state.json"))
		assert.False(t, ready)
		assert.NoError(t, err)
	})
}
