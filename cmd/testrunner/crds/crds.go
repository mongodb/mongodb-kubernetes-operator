...
func EnsureCreation(config *rest.Config, deployDir string) error {
	...
	if err != nil {
		return errors.Errorf("error creating apiextensions client set: %v", err)
	}

	crdFilePaths, err := allCrds(deployDir)
	if err != nil {
		return errors.Errorf("error walking deploy directory: %v", err)
	}
	...
	for _, filePath := range crdFilePaths {
		...
		if err := marshalCRDFromYAMLBytes(data, crd); err != nil {
			return errors.Errorf("error converting yaml bytes to CRD: %v", err)
		}
		...
		if err != nil {
			return errors.Errorf("error creating custom resource definition: %v", err)
		}
	}
	return nil
}
