...
func TestReplicaSetTLSRecreateMdbc(t *testing.T) {
	...
	t.Run("Create MongoDB Resource", mongodbtests.CreateMongoDBResource(ctx, &mdb1))
	...
	tester1, err := FromResource(ctx, mdb2)
	if err != nil {
		t.Fatal(err)
	}

	t.Run("Create MongoDB Resource", mongodbtests.CreateMongoDBResource(ctx, &mdb2))
